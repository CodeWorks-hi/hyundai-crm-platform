...
def build_system_prompt():
    ...
    try:
        for idx, faq in enumerate(FAQS, 1):
            ...
            formatted_answer = answer.strip().replace(newline, f'{newline}   - ')
            faq_items.append(
                f"{idx}. [{question.strip()}]{newline}"
                f"   - {formatted_answer}"
            )
...
def support_ui():
    ...
    for faq in FAQS:
        question, answer = faq.split("|||")
        with st.expander(question.strip(), expanded=False):
