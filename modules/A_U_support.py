...
TEMPLATE_SECRET_PATH = ".streamlit/secrets.toml"

# secrets.toml에서 key 불러오기
def get_custom_secret(key):
    try:
        if os.path.exists(TEMPLATE_SECRET_PATH):
            ...
            st.error("❌ secrets.toml 파일이 존재하지 않습니다.")
            return None
    except Exception as e:
        st.error(f"❌ 시크릿 로딩 실패: {e}")
        return None

# API Token 불러오기 (안전한 처리)
API_TOKEN = st.secrets.get("HUGGINGFACE_API_TOKEN")
if not API_TOKEN:
    API_TOKEN = get_custom_secret("HUGGINGFACE_API_TOKEN")
...
# HuggingFace API 응답 생성 (최적화된 코드)
def generate_gemma_response(user_input: str) -> str:
    if not API_TOKEN:
        return "❌ 시스템 오류: API 토큰이 설정되지 않았습니다. 관리자에게 문의해주세요."
...
def clean_response(text: str) -> str:
    text = re.sub(r'\*\*|```', '', text)
    text = re.sub(r'(\d+)만원', lambda x: f"{x.group(1)}만 원", text)
    return text[:1500].strip()

# Streamlit UI 구성 (기존과 동일)
def support_ui():
    ...
    <div class="faq-guide">
        ...
        <div style="color:#333;">
            ...
            • 자주 묻는 질문을 모아둔 <span style="color:#2A7FFF;">'❓ 자주 묻는 질문'</span> 섹션 확인 부탁드립니다.<br>
            • 이미 작성된 답변을 바탕으로 빠르게 문제 해결 가능합니다.
        </div>
        <div style="margin-top:10px; font-size:14px; color:#666;">
            ※ FAQ 질문 내용을 바탕으로 하단 입력창을 이용해 질문해주세요😍
        </div>
    </div>
    """, unsafe_allow_html=True)

    user_input = st.text_area("질문을 입력해 주세요", placeholder="예: 서비스 센터 방문 없이 정비가 가능한가요?", height=100)
