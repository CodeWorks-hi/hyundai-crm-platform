...
def leads_ui():
    ...
    if dealer_name == "" or dealer_id == "":
        ...
        with st.container():
            st.markdown("### 🔍 고객 리드 조회")
            st.markdown("고객 정보를 입력해주세요.")
            col_name, col_contact, col_button, col_rest = st.columns([1, 1, 1, 3])
            with col_name:
                selected_name = st.text_input("👤 고객 성명", key="leads_name")
            with col_contact:
                selected_contact = st.text_input("📞 고객 연락처", key="leads_contact")
            with col_button:
                st.markdown(" ")
                st.markdown(" ")
                submit = st.button("리드 확인", use_container_width=True)
